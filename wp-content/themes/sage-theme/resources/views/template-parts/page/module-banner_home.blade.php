<div class="container">
    <div class="bg-cover -left-2 right-0 top-0 bottom-0 xx:h-943 2xl:h-794 lg:h-802 md:h-839 absolute z-1" style="background-image:url({{$data->module['background']->url}})"
        alt="{{$data->module['background']->alt}}"></div>
    <div class="container lg:text-left absolute z-2 xx:top-19 xx:left-360 2xl:left-130 2xl:top-19 lg:top-19 lg:left-0 md:top-19 md:left-25">
        <div class="logo w-30-per 2xl:ml-1 lg:ml-43">
            <img src="{{$data->module['logo']->url}}" alt="{{$data->module['logo']->alt}}">
        </div>
        <div class="menu hidden lg:flex row w-52-per text-left absolute z-2 2xl:left-508 2xl:w-52-per 2xl:top-1 2xl:-ml-2 lg:top-0 lg:right-39 lg:w-57-per">
            <div class="btn-white w-1/4 font-semibold text-lg">
                <a class="no-underline" href="{{$data->module['about']->url}}" target="{{$data->module['about']->target}}">{{$data->module['about']->title}}</a>
            </div>
            <div class="btn-white w-1/4 font-semibold text-lg">
                <a class="no-underline" href="{{$data->module['leader']->url}}" target="{{$data->module['leader']->target}}">{{$data->module['leader']->title}}</a>
            </div>
            <div class="btn-white w-1/4 font-semibold text-lg pl-6">
                <a class="no-underline" href="{{$data->module['careers']->url}}" target="{{$data->module['careers']->target}}">{{$data->module['careers']->title}}</a>
            </div>
            <div class="btn-white w-1/4 font-semibold text-lg">
                <a class="no-underline" href="{{$data->module['contact']->url}}" target="{{$data->module['contact']->target}}">{{$data->module['contact']->title}}</a>
            </div>
        </div>
        ...
        <div class="pic xx:w-60-per">
            <div class="pic absolute lg:w-71-per md:w-111-per md:top-457 md:-right-110 xx:top-126 xx:-right-173 2xl:-right-152 2xl:top-109 lg:-right-192 lg:top-139">
                <img src="{{$data->module['image_1']->url}}" alt="{{$data->module['image_1']->alt}}"></div>
            <div class="phone absolute xx:top-370 xx:right-430 xx:w-16-per 2xl:right-464 2xl:top-353 lg:w-15-per lg:top-324 lg:right-254 md:w-24-per md:top-667 md:right-413">
                <img src="{{$data->module['image_2']->url}}" alt="{{$data->module['image_2']->alt}}"></div>
        </div>
